...
def to_sql(title, novel_id, author, novel_type, novel_link, content, date):
    title = title.replace("'", "''")
    author = author.replace("'", "''")
    novel_type = novel_type.replace("'", "''")
    novel_link = novel_link.replace("'", "''")
    ...
    query = '''
        insert into novel
        (novel_id, title, author, type, link, content, date)
        values
        (?, ?, ?, ?, ?, ?, ?)
    '''
    param = (novel_id, title, author, novel_type, novel_link, content, date)
    cursor.execute(query, param)
    cursor.close()
    conn.commit()
    conn.close()
